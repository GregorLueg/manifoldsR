...
Performs densMAP dimensionality reduction on the input data. densMAP is
UMAP with an added density-preserving term, so a tight cluster stays
tight and a diffuse one stays diffuse. Plain UMAP gives you no such
...
- knn:
  ...
  Optional `NearestNeighbours` class. If provided, densMAP will skip the
...
Setting `lambda` to `0` in
[`params_densmap()`](https://gregorlueg.github.io/manifoldsR/reference/params_densmap.md)
recovers plain
[`umap()`](https://gregorlueg.github.io/manifoldsR/reference/umap.md)
exactly. The density term is only active over the final `frac` of the
epochs, which is why it costs comparatively little.
